...
![A post shared in our WhatsApp community](wa.png)
...
- [`/ash`](/ash)  is a lightweight [matrix] client that watches all my chats (WhatsApp is where most of the club groups are at)
- [`/lava`](/lava) is a HTML to markdown parser that works over an API, it uses the Defuddle (Obsidian Web Clipper) library for all the heavy work
- [`linkstash.hsp-ec.xyz`](https://linkstash.hsp-ec.xyz) our club linkstash ^^

Each of these tools work independently, but When combined with [/lava](https://polarhive.net/lava) (a daemon for clipping markdown) and [/ash](https://polarhive.net/ash) ([matrix] client for link extraction), it forms a complete personal web archiving pipeline.
...
[/ash](/ash) is a small [matrix] bot (written in Go) that monitors rooms, extracts URLs from messages, and forwards them (via webhook) to Linkstash or other services.

> Matrix is a federated, open protocol for real-time communication. Rooms have event histories and, for rooms the bot is in, [/ash](/ash) can read events, parse URLs, and capture links without relying on any third-party scraping service.

- [/ash](/ash) will read configured rooms off `config.json` and filter messages by room
...
2. Collect: [/ash](/ash) extracts URLs and forwards them to Linkstash if they're on a whitelist
